...
export default function AddExpenseForm({closeModal} : {closeModal : () => void}) {
    const {id} = useParams() // Asi obteines los params desde client y en AddExpenseForm({params}) - asi obtienes en server
    const createExpenseWithBudgetId = createExpense.bind(null, id.toString())    
    ...
    return (
        <>
            ...
            >
                ...
                <input
                    type="submit"
                    className="bg-amber-500 w-full rounded-md p-3 text-white uppercase font-bold hover:bg-amber-600 cursor-pointer transition-colors"
